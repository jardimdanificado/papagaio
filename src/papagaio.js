...
function applyEvalExpressions(p, text, evals) {
  ...
  // Aplica os evals em ordem reversa para manter os índices corretos
  for (let i = evals.length - 1; i >= 0; i--) {
    const placeholder = `__EVAL_${i}__`;
    const evalResult = evaluateCode(p, evals[i].code);
...
function evaluateCode(p, code) {
  try {
    return String(Function("papagaio", "ctx", `"use strict";return(function(){${code}})();`)(p, {}));
  } catch (e) {
    return "";
  }
}
